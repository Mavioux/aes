...
const int Galois_Mul_LUT[14][256] = {
...
class AES {   
    ...
    public:
        ...
        void Decrypt(int ciphertext[16], int key[16], int plaintext[16]) {
            int state[16];
            int temp_keys[11][16];
            int keys[11][16];
        ...
        void AddRoundKey(int state[], int key[]) {
            ...
            std::memcpy(state, temp, 16 * sizeof(int));
        }
        ...
        void SubBytesInverse(int state[]) {
            for(uint8_t i = 0; i < 16; i++) {
                state[i] = inv_sbox[state[i] / 16][state[i] % 16];
            }
        }

        void ShiftRowsInverse(int state[]) {
            int temp[16];
            for(uint8_t i = 0; i < 4; i++) {
                temp[i] = state[i];
            }
        ...
        void MixColumnsLUTInverse(int state[]){
            int row[4];
            for(uint8_t i = 0; i < 4; i++) {
                for(uint8_t j = 0; j < 4; j++) {
                    row[j] = state[4 * j + i];
        ...
        void MixColumnLUTInverse(int r[], int i) {
            int row[4];
            memcpy(row, r, 4 * sizeof(int));
            for(uint8_t j = 0; j < 4; j++) {
                int result = 0;
